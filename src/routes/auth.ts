...
import { z } from 'zod'
import { UserDB, LogDB } from '../lib/db'
...
router.post('/bootstrap', async (req: Request, res: Response) => {
  ...
  try {
    const adminExists = await UserDB.findOne({ role: { $in: ['admin', 'commercial'] } })
    if (adminExists) {
    ...
    const user = await UserDB.create({
      nom, email: email.toLowerCase(), passwordHash, role: 'admin',
      organisation: 'NeoTravel', emailVerified: true,
    })

    await LogDB.create({ action: 'ADMIN_BOOTSTRAP', status: 'success', message: `Premier compte admin créé : ${nom} (${email})` })
...
router.post('/register', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const passwordHash = await bcrypt.hash(password, 12)
    const user = await UserDB.create({ nom, email: email.toLowerCase(), passwordHash, role: 'client', organisation })

    await LogDB.create({ action: 'USER_REGISTERED', status: 'success', message: `Nouveau compte client créé : ${nom} (${email})` })

    sendWelcomeEmail(user).catch(() => {})

    const token = signToken(user.id, user.role)
    res.status(201).json({ token, user: { id: user.id, nom: user.nom, email: user.email, role: user.role, organisation: user.organisation } })
  } catch (err) {
...
router.post('/staff', requireAuth, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const passwordHash = await bcrypt.hash(password, 12)
    const user = await UserDB.create({ nom, email: email.toLowerCase(), passwordHash, role, organisation })

    await LogDB.create({ action: 'STAFF_CREATED', status: 'success', message: `Compte NeoTravel créé : ${nom} (${email}) — rôle ${role}`, payload: { createdBy: req.userId } })

    res.status(201).json({ user: { id: user.id, nom: user.nom, email: user.email, role: user.role, organisation: user.organisation } })
  } catch (err) {
...
router.post('/login', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await LogDB.create({ action: 'USER_LOGIN', status: 'success', message: `Connexion réussie : ${user.email}` })

    const token = signToken(user.id, user.role)
    res.json({ token, user: { id: user.id, nom: user.nom, email: user.email, role: user.role, organisation: user.organisation } })
  } catch (err) {
...
router.get('/me', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({ id: user.id, nom: user.nom, email: user.email, role: user.role, organisation: user.organisation })
  } catch {
...
router.post('/forgot-password', async (req: Request, res: Response) => {
  ...
  try {
    const user = await UserDB.findOne({ email: parsed.data.email })
...
router.post('/reset-password', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user) { res.status(400).json({ message: 'Lien invalide ou expiré. Veuillez refaire une demande.' }); return }
    ...
    await LogDB.create({ action: 'PASSWORD_RESET', status: 'success', message: `Mot de passe réinitialisé pour ${user.email}` })
...
router.patch('/me', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (updates.email) {
      const exists = await UserDB.findOneExcludeId({ email: updates.email }, req.userId!)
    ...
    await LogDB.create({ action: 'USER_PROFILE_UPDATED', status: 'success', message: `Profil mis à jour : ${user.email}`, payload: updates }).catch(() => {})
...
router.patch('/password', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await UserDB.update(user.id, { passwordHash: await bcrypt.hash(parsed.data.newPassword, 12) })

    await LogDB.create({ action: 'USER_PASSWORD_UPDATED', status: 'success', message: `Mot de passe modifié : ${user.email}` }).catch(() => {})
