...
2. **Appel Claude via le Vercel AI SDK** (`streamText` de `'ai'` + provider `anthropic` de `'@ai-sdk/anthropic'`, modèle `claude-sonnet-4-6`, `maxTokens: 1200`) avec un **system prompt très strict** (voir 4.3). ⚠️ **Mise à jour (vérifiée à date de cette doc) :** la route a été migrée depuis l'appel direct au SDK Anthropic (`@anthropic-ai/sdk`) vers le **Vercel AI SDK**. La réponse est désormais **streamée en SSE** (`Content-Type: text/event-stream`) : le texte du message de l'IA est extrait caractère par caractère pendant la génération et renvoyé au client au fur et à mesure (effet "l'assistant tape en direct"), via une state machine qui isole le champ `"message"` du JSON brut en cours de génération (`route.ts:182-234`).
3. **Parsing JSON** du texte complet une fois le stream terminé (regex `\{[\s\S]*\}` puis `JSON.parse`), avec récupération en cas d'échec.
...
7. **Logging** : chaque appel (latence, tokens, action, confiance, erreurs) est loggé en console **et** envoyé au backend via `POST /api/logs` (fire-and-forget, non bloquant). Les tokens d'usage sont lus via `result.usage` du SDK Vercel (`promptTokens` / `completionTokens`).

> Note d'audit : ce fichier contient `// @ts-nocheck` en première ligne, qui désactive la vérification TypeScript dessus — probablement temporaire le temps de stabiliser la migration vers le Vercel AI SDK. À lever une fois la migration jugée stable, pour retrouver la sécurité de typage du reste du projet.
...
## 4.5 Relances automatiques (n8n) — constaté côté frontend

Le dashboard commercial (`src/app/dashboard/leads/[id]/page.tsx`) affiche un encart **"Relances automatiques (n8n)"** qui montre :

- Un compteur de relances envoyées sur un devis : `Relances n8n : {reminderCount}/2 envoyées` (`page.tsx:1094`), visible quand `quote.statut_devis === 'sent'` et `reminderCount < 2`.
- Le statut de chaque relance (1 puis 2), avec un calcul d'échéance (`relance 2 : après relance 1 + 72h`) et un message `"(dû — n8n va envoyer prochainement)"` quand l'échéance est dépassée mais l'envoi pas encore confirmé.
- Le statut `devis_valide` dans `LeadStatus` porte un commentaire explicite : `// approuvé par commercial, n8n va envoyer` (`src/types/index.ts:7`).

**Ce que ça confirme :** une fois qu'un devis est validé et envoyé, le système relance automatiquement le client (jusqu'à 2 relances, espacées dans le temps) **sans action manuelle du commercial** — l'outil d'automatisation utilisé est **n8n** (plateforme de workflow d'automatisation), qui tourne côté backend/infrastructure et n'est pas visible dans ce repo frontend. Le frontend ne fait qu'**afficher l'état** de ces relances (lecture seule), il ne déclenche rien lui-même.

**Limite de cette doc :** le workflow n8n exact (déclencheur, contenu des emails, conditions d'arrêt) n'est pas dans ce dépôt et n'a donc pas pu être audité ici — seule sa présence et son rôle fonctionnel sont confirmés par les références dans le code frontend.
